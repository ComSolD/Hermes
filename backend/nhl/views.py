...
from .models import NHLMatch, NHLPlayer, NHLPlayerStat, NHLTeam, NHLTeamStat
from .serializers import NHLMatchSerializer, NHLTotalSerializer, NHLMoneylineSerializer, NHL1x2Serializer, NHLHandicapSerializer, NHLMatchesSchedule, NHLPlayerStatisticSerializer, NHLTeamStatisticSerializer, NHLStandingsSerializer # Импортируем сериализатор матча
...
def match_statistic(request, match_id):
    ...
    # Сериализуем данные
    serializer = NHLMatchSerializer(match, context={"request": request})
...
def match_total(request, match_id, period):
    ...
    # Сериализуем данные
    serializer = NHLTotalSerializer(match, context={"period": period, "request": request})
...
def match_moneyline(request, match_id):
    ...
    # Сериализуем данные
    serializer = NHLMoneylineSerializer(match, context={"request": request})
...
def match_1x2(request, match_id):
    ...
    # Сериализуем данные
    serializer = NHL1x2Serializer(match, context={"request": request})
...
def match_handicap(request, match_id, period):
    ...
    # Сериализуем данные
    serializer = NHLHandicapSerializer(match, context={"period": period, "request": request})
...
def seasons_by_filters(request):
    ...
    # 3. Фильтрация матчей
    matches = NHLMatch.objects.filter(filters).exclude(stage__isnull=True)

    # 4. Если выбран игрок — ограничиваем по его матчам
    if data.get("player_id"):
        player_match_ids = NHLPlayerStat.objects.filter(
    ...
    teamstat_qs = NHLTeamStat.objects.filter(match_id__in=match_ids)
...
def stages_by_filters(request):
    ...
    # Промежуточный фильтр NHLTeamStat по статусу и (если есть) команде
    ...
    filtered_match_ids = NHLTeamStat.objects.filter(teamstat_filters).values_list("match_id", flat=True)

    # Получение стадий
    raw_stages = NHLMatch.objects.filter(match_id__in=filtered_match_ids).values_list("stage", flat=True).distinct()
    stage_choices_dict = dict(NHLMatch._meta.get_field("stage").choices)
...
def homeaway_by_filters(request):
    ...
    # 2. Фильтрация по NHLTeamStat и получение статусов
    teamstat_qs = NHLTeamStat.objects.filter(match_id__in=match_ids)
    ...
    status_dict = dict(NHLTeamStat.STATUS_CHOICES)
...
def opponents_by_filters(request):
    ...
    # 6. Промежуточный фильтр NHLTeamStat по status и team_id
    teamstat_filters = Q(match_id__in=match_ids, team_id=team_id)
    if data.get("homeaway"):
        teamstat_filters &= Q(status=data["homeaway"])

    relevant_match_ids = NHLTeamStat.objects.filter(teamstat_filters).values_list("match_id", flat=True)

    # 7. Повторный фильтр матчей, чтобы найти оппонентов
    relevant_matches = NHLMatch.objects.filter(match_id__in=relevant_match_ids)
    ...
    opponents = NHLTeam.objects.filter(team_id__in=opponent_ids)
    serializer = NHLTeamStatisticSerializer(opponents, many=True)
...
def teams_by_filters(request):
    ...
    teams = NHLTeam.objects.filter(team_id__in=team_ids)

    # 4. Сериализация
    serializer = NHLTeamStatisticSerializer(teams, many=True)
...
def players_by_filters(request):
    ...
    if data.get("homeaway"):
        ...
        valid_team_stats = NHLTeamStat.objects.filter(
            match_id__in=match_ids,
        ...
        player_ids = NHLPlayerStat.objects.filter(player_stats_filter).filter(
            Q(*[
        ...
        player_ids = NHLPlayerStat.objects.filter(player_stats_filter).values_list("player_id", flat=True).distinct()


    # 5. Получаем игроков
    players = NHLPlayer.objects.filter(player_id__in=player_ids)

    # 6. Сериализация
    serializer = NHLPlayerStatisticSerializer(players, many=True)
...
def standings(request):
    ...
    matches = NHLMatch.objects.filter(
        season=season,
    ...
    serializer = NHLStandingsSerializer(teams, many=True, context={"request": request, "season": season})
...
@api_view(['GET'])
def seasons(request):
    seasons = NHLMatch.objects.values_list("season", flat=True).distinct()
