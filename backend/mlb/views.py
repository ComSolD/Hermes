...
def seasons_by_filters(request):
    ...
    # 3. Фильтрация матчей
    matches = MLBMatch.objects.filter(filters).exclude(stage__isnull=True)

    # 4. Если выбран игрок — ограничиваем по его матчам
    if data.get("player_id"):
        player_match_ids = MLBPlayerStat.objects.filter(
    ...
    teamstat_qs = MLBTeamStat.objects.filter(match_id__in=match_ids)
    ...
    # 8. Получаем сезоны из MLBMatch
    seasons = MLBMatch.objects.filter(match_id__in=filtered_match_ids).values_list("season", flat=True).distinct()
...
def stages_by_filters(request):
    ...
    # Промежуточный фильтр MLBTeamStat по статусу и (если есть) команде
    ...
    filtered_match_ids = MLBTeamStat.objects.filter(teamstat_filters).values_list("match_id", flat=True)

    # Получение стадий
    raw_stages = MLBMatch.objects.filter(match_id__in=filtered_match_ids).values_list("stage", flat=True).distinct()
    stage_choices_dict = dict(MLBMatch._meta.get_field("stage").choices)
...
def homeaway_by_filters(request):
    ...
    # 2. Фильтрация по MLBTeamStat и получение статусов
    teamstat_qs = MLBTeamStat.objects.filter(match_id__in=match_ids)
    ...
    status_dict = dict(MLBTeamStat.STATUS_CHOICES)
...
def opponents_by_filters(request):
    ...
    # 6. Промежуточный фильтр MLBTeamStat по status и team_id
    teamstat_filters = Q(match_id__in=match_ids, team_id=team_id)
    if data.get("homeaway"):
        teamstat_filters &= Q(status=data["homeaway"])

    relevant_match_ids = MLBTeamStat.objects.filter(teamstat_filters).values_list("match_id", flat=True)

    # 7. Повторный фильтр матчей, чтобы найти оппонентов
    relevant_matches = MLBMatch.objects.filter(match_id__in=relevant_match_ids)
    ...
    opponents = MLBTeam.objects.filter(team_id__in=opponent_ids)
    serializer = MLBTeamStatisticSerializer(opponents, many=True)
...
def teams_by_filters(request):
    ...
    # 4. Сериализация
    serializer = MLBTeamStatisticSerializer(teams, many=True)
...
def players_by_filters(request):
    ...
    if data.get("homeaway"):
        ...
        valid_team_stats = MLBTeamStat.objects.filter(
            match_id__in=match_ids,
        ...
        player_ids = MLBPlayerStat.objects.filter(player_stats_filter).filter(
            Q(*[
        ...
        player_ids = MLBPlayerStat.objects.filter(player_stats_filter).values_list("player_id", flat=True).distinct()


    # 5. Получаем игроков
    players = MLBPlayer.objects.filter(player_id__in=player_ids)

    # 6. Сериализация
    serializer = MLBPlayerStatisticSerializer(players, many=True)
