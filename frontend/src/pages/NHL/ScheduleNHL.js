...
function ScheduleNHL() {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/nhl/schedule/?date=${dateParam}`)
        ...
        <div className="schedule-info">
          ...
          {matches.length > 0 ? (
            matches.map((match) => {
              ...
              return (
                <Link to={`/nhl/match/${match.match_id}`} key={match.match_id} className="match-item">
                  <span className={homeWin ? "winner" : "loser"}>
                    {match.match_info.home_team}
                  </span>
                  <span className="match-score">
                    {homeScore}:{awayScore}{match.match_info.match_status !== "Maintime" ? ` ${match.match_info.match_status}` : ""}
                  </span>
                  <span className={!homeWin ? "winner" : "loser"}>
