...
function StandingsMLB() {
  ...
  const leagueOptions = [
    ...
    { value: "American League", label: "Американская Лига" },
    { value: "National League", label: "Национальная Лига" },
  ];

  useEffect(() => {
    fetch('http://localhost:8000/api/mlb/seasons/')
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(`http://127.0.0.1:8000/api/mlb/standings/?${params.toString()}`)
      .then((res) => res.json())
